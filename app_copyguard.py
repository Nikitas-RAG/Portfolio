...
2. If the input is raw HTML code, preserve all formatting tags (like <strong>, <em>, <a>) but audit the text and hyperlinks inside them.
3. If F1 or F2 is violated, set status to "RED", list fatal_blocks, and leave optimized_copy blank.
4. If formatting/encoding rules are violated, fix them, set status to "YELLOW", list warnings, and output optimized_copy (retaining HTML structure if input was HTML).
5. If perfectly compliant, set status to "GREEN" and output optimized_copy.
"""
...
st.markdown("Ensure your SMS and Email copy bypasses carrier firewalls and ISP spam filters. **(Powered by Gemini 2.5 Flash)**")
...
# Context-Specific Directions Helper
if channel == "Email":
    st.info("💡 **Email Pro-Tip:** To audit hyperlinks, bold text, and italics, copy and paste the **HTML Source Code** directly from your GHL/CRM email editor!")
else:
    st.info("💡 **SMS Pro-Tip:** Emojis and smart quotes force UCS-2 carrier encoding, which triples your text messaging costs. CopyGuard will auto-sanitize these out!")

# Main Input Area
payload = st.text_area(
    "Paste your marketing copy or raw HTML code here:", 
    height=250, 
    placeholder="<p>Hey {{first_name}}, click <a href='http://bit.ly/trial'>here</a> to claim your <strong>14-day trial</strong>! 🚀</p>" if channel == "Email" else "Hey {{first_name}}, claim your trial today! 🚀 -> bit.ly/trial"
)
    ...
    if not api_key:
        ...
        with st.spinner("Auditing against 2026 Telecom & ISP regulations..."):
            try:
                ...
                # Automatically strip out all [cite:X] markers safely
                raw_text = re.sub(r'\[cite:\s*\d+\]', '', raw_text)
                ...
                if status == "RED":
                    ...
                    st.code(audit_result.get("optimized_copy", ""), language="html" if channel == "Email" else "text")
